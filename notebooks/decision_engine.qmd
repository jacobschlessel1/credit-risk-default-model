...
To get a sense of how many loans may be considered "risky", we will create 
different bins of probabilities of defaulting to see how many loans fall 
into each bin.
...
There will be 3 levels of risk tolerance in deciding which policy to 
enact: Conservative, Balanced, and Aggressive, with PD cutoffs of 5%, 10%, 
and 20% respectively.
...
Conservative policy controls risk but limits growth, as only 8% of 
applicants would have been accepted under this policy. The Balanced policy 
is slightly more realistic with an acceptance rate of about 20%, while the 
aggressive policy grants loans to over 50% of applicants. While these 
approval rates provide a quick idea about risk tolerance for each policy, 
these are by no means all-encompassing decision-makers about loan 
acceptance strategy. A lender could compensate for the higher risk under  
the aggressive policy, for example, by charging substantially higher 
interest rates to borrowers who are deemed to be more likely to default, 
in which case the real approval rate would be much higher than the 53% proposed 
by this decision engine.

## Tables for Dashboard

```{python}
# Dashboard of loans
dashboard_loans = df_scored[[
    "issue_d",
    "loan_amnt",
    "calibrated_pd",
    "risk_bucket"
...
dashboard_loans["issue_year"] = dashboard_loans["issue_d"].dt.year
...
portfolio_summary["loan_share"] = (
    portfolio_summary["n_loans"] /
    portfolio_summary["n_loans"].sum()
)

portfolio_summary["exposure_share"] = (
    portfolio_summary["exposure"] /
    portfolio_summary["exposure"].sum()
)
# Reset index of policy summary
policy_summary_reset = policy_summary.reset_index()
```

## Save tables 

```{python}
dashboard_loans.to_parquet(
    "../data/dashboard/dashboard_loans.parquet",
    index=False,
    compression="snappy"
)

portfolio_summary.to_parquet(
    "../data/dashboard/portfolio_summary.parquet",
    index=False,
    compression="snappy"
)

policy_summary_reset.to_parquet(
    "../data/dashboard/policy_summary.parquet",
    index=False,
    compression="snappy"
)
```