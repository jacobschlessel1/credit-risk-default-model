...
date available) were just slightly higher than the pre-2016 loans for each 
respective model. Once again, the XGBoost model captured more true 
...
```{python}
# Use the trained post-2016 XGBoost model
# Predict PDs on the full post-2016 dataset (not just test set)
pd_post2016 = xgb_post2016.predict_proba(X)[:, 1]

pd_dist = pd.Series(pd_post2016, name="predicted_pd")

# Summary statistics
print("Predicted Probability of Default — Post-2016 Non-Current Loans\n")
print(pd_dist.describe(percentiles=[0.01, 0.05, 0.10, 0.25, 0.50, 0.75, 0.90, 0.95, 0.99]))

# Mean vs observed default rate
observed_default_rate = df_post2016["default"].mean()

print("\nObserved Default Rate:", round(observed_default_rate, 4))
print("Average Predicted PD:", round(pd_dist.mean(), 4))

plt.figure(figsize=(10, 6))
plt.hist(pd_dist, bins=50)
plt.xlabel("Predicted Probability of Default")
plt.ylabel("Number of Loans")
plt.title("Distribution of Predicted PDs\nPost-2016 Non-Current Loans")
plt.tight_layout()
plt.show()
...
risk_buckets = pd.cut(pd_dist, bins=bins, labels=labels)

print("\nRisk Bucket Distribution:")
print(risk_buckets.value_counts(normalize=True).sort_index())
```

The XGBoost model outputs values between 0 and 1, but these numbers are 
not necessarily representative of raw probablities of default. Rather, 
these values can more accurately be described as relative scores assigned 
to each loan for how likely they are to default compared to other loans 
in the dataset. The benefit of this is that it generates quality 
predictions for whether a loan will default or not (binary outcome), but 
the tradeoff is that it does not generate interpretable probablitlies. In 
order to get the model to output probabilities that are more accurate to 
the real-world, it needs to be calibrated so that it aligns with the 
number of probabilities actually observed in the data. To accomplish this, 
we will use isotonic calibration.
...
# Fit calibrated model, predict using all data (training and testing) 
calibrated_xgb.fit(X_train, y_train)
pd_cal = calibrated_xgb.predict_proba(X)[:, 1]

pd_calibrated = pd.Series(pd_cal, name="predicted_pd_calibrated")

plt.figure(figsize=(10, 6))
plt.hist(pd_calibrated, bins=50)
plt.xlabel("Predicted Probability of Default")
plt.ylabel("Number of Loans")
plt.title("Distribution of Predicted PDs\nPost-2016 Non-Current Loans")
plt.tight_layout()
...
print("Calibrated Probability of Default — Post-2016 Non-Current Loans\n")

print(
    pd_calibrated_series.describe(
...
print("\nObserved Default Rate:", round(observed_default_rate, 4))
print("Average Calibrated PD:", round(pd_calibrated_series.mean(), 4))

risk_buckets = pd.cut(pd_calibrated, bins=bins, labels=labels)

print("\nRisk Bucket Distribution:")
print(risk_buckets.value_counts(normalize=True).sort_index())
```

It might intially seem problematic that this model is being applied to 
all post-2016, non-current loans since most of these loans (80%) were 
used to train the model. However, leakage is not a concern for the 
application of assigning probabilities and creating a decision engine 
because we are not evaluating the model's performance (that was done 
using the testing set), we are simply using the model's predictions to 
simulate decision-making and seeing summarizing how it can be used to 
guide policy.
...
# Save trained post-2016 XGBoost model
joblib.dump(
    xgb_post2016,
    ARTIFACT_DIR / "xgb_post2016.pkl"
)

# Save calibrated model
joblib.dump(
    calibrated_xgb,
    ARTIFACT_DIR / "xgb_post2016_calibrated.pkl"
)

# Save feature list 
joblib.dump(
    X.columns.tolist(),
    ARTIFACT_DIR / "model_features_post2016.pkl"
)
